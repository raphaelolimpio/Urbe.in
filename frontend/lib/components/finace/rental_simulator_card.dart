...
class RentalSimulatorCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final barColor = pctDiff > 0.2 ? Colors.red : (pctDiff < -0.2 ? Colors.orange : Colors.green);
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              Text("Renda Bruta (Aluguel)", style: TextStyle(color: Colors.grey[600], fontSize: 12)),
              Text(
                AppFormatters.formatCurrency(aluguelSimulado),
                style: TextStyle(color: colorProfit, fontWeight: FontWeight.bold, fontSize: 16),
              ),
            ],
          ),
          SliderTheme(
          ...
          Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text("Média da Região (IA):", style: TextStyle(fontSize: 11, color: Colors.grey[600])),
                Row(
                  children: [
                    Text(AppFormatters.formatCurrency(mediaMercado), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                    const SizedBox(width: 6),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                      decoration: BoxDecoration(
                        color: pctDiff > 0.1 ? Colors.red[50] : (pctDiff < -0.1 ? Colors.green[50] : Colors.blue[50]),
                        borderRadius: BorderRadius.circular(4)
                      ),
                      child: Text(
                        pctDiff > 0.1 ? "Acima" : (pctDiff < -0.1 ? "Oportunidade" : "Na Média"),
                        style: TextStyle(
                          fontSize: 10, 
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                Row(
                  children: [
                    Text("Despesas Operacionais", style: TextStyle(color: Colors.grey[700])),
                ...
                Text(
                  ...
                  style: TextStyle(color: colorLoss, fontWeight: FontWeight.bold),
                ),
              ],
            ),
          ),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: noiMensal >= 0 ? colorProfit.withOpacity(0.1) : colorLoss.withOpacity(0.1),
              borderRadius: BorderRadius.circular(8),
            ),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text("Resultado Líquido (Mês)", style: TextStyle(fontWeight: FontWeight.bold)),
                Text(
